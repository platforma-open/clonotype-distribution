...
- **Discrete-filter derivation.** Compute `pl7.app/discreteValues` locally for the `Dominant Group`, `Consensus Dominant`, and `Peak Timepoint` output specs, so Lead Selection dropdowns work regardless of whether upstream annotated its columns. Scan the grouping column's data in Tengo via `col.data.getDataAsJson()` for Dominant Group / Consensus Dominant (capped at 150 uniques); use `args.timepointOrder` directly for Peak Timepoint. Lead Selection now renders these columns as dropdowns listing the actual categories (e.g. `spleen`, `lung`, `mediastinal lymph node`) instead of free-text fields.
